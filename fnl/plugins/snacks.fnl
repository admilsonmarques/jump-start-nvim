...
(fn dashboard-sections []
  (let [snacks (require :snacks)
        ...
        cmds [{:title "My Work"
               :cmd "gh pr list --json title,number,updatedAt -L 10 | jq -r '.[] | \"#\" + (.number|tostring) + \" \" + .title + \" \" + (.updatedAt[0:10])'"
               :action (fn []
                         (let [snacks (require :snacks)]
    ...
    [;; --- PANE 1 (Left) ---
     ...
     ;; Map the git commands into terminal sections for the second pane
     (core.map (fn [cmd]
                 (core.merge {:pane 2
                              :section :terminal
                              :enabled in-git?
...
{1 :folke/snacks.nvim
 ...
 :keys [{1 :<leader>ff
        ...
        {1 :<leader>fp
         ...
         :desc :Projects}
         {1 "<leader>;"
         2 (fn [] (let [snacks (require :snacks)] (snacks.dashboard.open)))
         :desc "Open Dashboard"}]}
